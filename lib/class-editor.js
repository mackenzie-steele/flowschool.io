...
function renderTeach(id) {
  ...
  document.getElementById('tv-sections').innerHTML = SECTIONS.map(function (s) {
    ...
    // the poem folds under its flow — there when wanted, quiet when not
    var poem = flow ? poemFor(flow.id) : null;
    var lines = poem ? poemLines(poem) : [];
    var poemHtml = lines.length
      ? '<button type="button" class="tv-poem-toggle" aria-expanded="false">' +
          '<span class="material-symbols-sharp i-sm">expand_more</span>Poem of the flow</button>' +
        '<div class="tv-poem" hidden>' +
  ...
  document.querySelectorAll('#tv-sections .tv-poem-toggle').forEach(function (btn) {
    btn.addEventListener('click', function () {
      var open = btn.getAttribute('aria-expanded') !== 'true';
      fsUnfold(btn.nextElementSibling, open);
...
// the poem of the flow — a V/YBP/D worksheet linked to this flow.
// Newest wins if several are linked. Cue building mirrors the
// worksheet's own buildCue so both surfaces speak identical lines.
function poemFor(flowId) {
  try {
    var store = JSON.parse(localStorage.getItem('fs-cue-flows') || '{}');
    var hits = Object.keys(store).map(function (k) { return store[k]; })
      .filter(function (e) { return e && e.flowId === flowId; })
      .sort(function (a, b) { return new Date(b.savedAt) - new Date(a.savedAt); });
    return hits[0] || null;
  } catch (_) { return null; }
}

function poemLines(entry) {
  return (entry.rows || []).map(function (r) {
    ...
    var cue = [verb, bodyNorm, dir].filter(Boolean).join(' ');
    if (cue.slice(-1) !== '.') cue += '.';
