...
// Caret-anchored panel over an auto-growing textarea. Tab and Enter both
// insert the top (or armed) match, arrows arm, Esc dismisses for the
// current token (Esc first if you want a literal newline mid-token). cfg:
...
function fsShorthandSuggest(cfg) {
  ...
  function onKey(ev) {
    ...
    if (ev.key === 'Tab') {
      ...
      ev.preventDefault();
      insert(state.sel === -1 ? 0 : state.sel); // top match while typing; armed row wins
